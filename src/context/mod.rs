use crate::{
    cache::PathCache, renderer::TextureType, DrawInfo, LineCap, LineJoin, Paint, Path,
    PathCommands, PathSlice, RendererDevice,
};
...
impl<'a, R: RendererDevice> PathRefWithCache<'a, R> {
    fn new(path: &'a Path<R>, paint: &'a Paint) -> Self {
        Self {
            path,
...
impl<'a, R: RendererDevice> Drop for PathRefWithCache<'a, R> {
    fn drop(&mut self) {
        self.path.inner.borrow_mut().draw_info = Some(std::mem::take(&mut self.draw_info));
    }
}
...
impl<'a, R: RendererDevice> FlattenExpandPath for PathRefWithCache<'a, R> {
    ...
    fn expand_stroke(
        ...
        self.cache.expand_stroke(
            ...
            miter_limit,
            tess_tol,
            &mut self.draw_info.stroke,
        );
    }

    #[inline]
    fn expand_lines(&mut self) {
        self.cache.expand_lines(&mut self.draw_info.lines);
    }

    #[inline]
    fn get_fill_slice(&self) -> &[PathSlice] {
        return &self.draw_info.fill;
    }

    #[inline]
    fn get_stroke_slice(&self) -> &[PathSlice] {
        return &self.draw_info.stroke;
    }

    #[inline]
    fn get_lines_slice(&self) -> &[PathSlice] {
        return &self.draw_info.lines;
    }
}
